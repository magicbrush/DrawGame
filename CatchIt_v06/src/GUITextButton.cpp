...
void GUITextButton::pressed( int x, int y )
{
  bool bIn = inside(x,y);  
  if(bIn)
  {
...
void GUITextButton::dragged( int x, int y )
{  
  if(bPressed)
  {
    bool bIn = inside(x,y);
...
void GUITextButton::release( int x, int y )
{
  if(bPressed)
  {
    bool bIn = inside(x,y);
...
void GUITextButton::customDraw()
{
  ...
  else
  {
    ...
    DrawGame::drawScaledStringByFont(Title,pFont,R);
  }
  ofPopStyle();
